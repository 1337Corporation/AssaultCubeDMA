...
public:
	...
	**/
	...
	{
		uintptr_t ntos_shutdown = TargetProcess.GetExportTableAddress("NtShutdownSystem", "csrss.exe", "ntoskrnl.exe");
		...
		*reinterpret_cast<uint64_t*>(jmp_bytes + 6) = function;
		...
		if (!TargetProcess.Read(ntos_shutdown, (PBYTE)orig_bytes, sizeof(orig_bytes), 4))
			return buffer;

	
		if (!TargetProcess.Write(ntos_shutdown, jmp_bytes, sizeof(jmp_bytes), 4))
		{
		...
		//Restore function
		if (!TargetProcess.Write(ntos_shutdown, orig_bytes, sizeof(orig_bytes), 4))
			LOG("[!] Failed to write memory at 0x%p\n", ntos_shutdown);
