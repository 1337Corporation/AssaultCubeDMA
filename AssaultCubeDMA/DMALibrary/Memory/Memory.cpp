...
uintptr_t Memory::GetExportTableAddress(std::string import, std::string process, std::string module)
{
	...
	bool result = VMMDLL_Map_GetEATU(TargetProcess.vHandle, TargetProcess.GetPidFromName(process) /*| VMMDLL_PID_PROCESS_WITH_KERNELMEMORY*/, const_cast<LPSTR>(module.c_str()), &eat_map);
	if (!result)
	{
...
uintptr_t Memory::GetImportTableAddress(std::string import, std::string process, std::string module)
{
	...
	bool result = VMMDLL_Map_GetIATU(TargetProcess.vHandle, TargetProcess.GetPidFromName(process) /*| VMMDLL_PID_PROCESS_WITH_KERNELMEMORY*/, const_cast<LPSTR>(module.c_str()), &iat_map);
	if (!result)
	{
